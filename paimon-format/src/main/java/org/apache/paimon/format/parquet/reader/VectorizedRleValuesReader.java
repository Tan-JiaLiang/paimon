...
public final class VectorizedRleValuesReader extends ValuesReader
                    ...
                    case RLE:
                        {
                            if (currentValue == state.maxDefinitionLevel) {
                                ...
                                nulls.setNulls(state.valueOffset, n);
                            }
                            state.valueOffset += n;
                            break;
                        }
                    case PACKED:
                        {
                            for (int i = 0; i < n; ++i) {
                                ...
                                if (currentValue == state.maxDefinitionLevel) {
                                    ...
                                    nulls.setNullAt(state.valueOffset++);
                                }
                            }
                            break;
                        }
     ...
     *
     * <p>The related states such as row index, offset, number of rows left in the batch and page,
     * are tracked by 'state'. The type-specific 'updater' is used to update or skip values.
     */
    ...
    private static class DefLevelProcessor {
        ...
        private final WritableIntVector defLevels;
            ...
            case RLE:
                {
                    ...
                    state.valueOffset += n;
                    defLevels.setInts(state.levelOffset, n, currentValue);
                    break;
                }
            case PACKED:
                {
                    for (int i = 0; i < n; ++i) {
                        ...
                        defLevels.setInt(state.levelOffset + i, currentValue);
                    }
                    break;
                }
        }
    ...
    private void skipValues(
        ...
        while (n > 0) {
            ...
            switch (mode) {
                case RLE:
                    {
                        // We only need to skip non-null values from `valuesReader` since nulls are
                        // represented
    ...
    public byte readByte() {
        throw new RuntimeException("_LEGACY_ERROR_TEMP_3187");
    }
    ...
    public void readBooleans(int total, WritableBooleanVector c, int rowId) {
        ...
        while (left > 0) {
            ...
            switch (mode) {
                case RLE:
                    c.setBooleans(rowId, n, currentValue != 0);
                    break;
                case PACKED:
                    {
                        for (int i = 0; i < n; ++i) {
                            // For Boolean types, `currentBuffer[currentBufferIdx++]` can only be 0
                            // or 1
                            c.setBoolean(rowId + i, currentBuffer[currentBufferIdx++] != 0);
                        }
                        break;
                    }
            }
    ...
    public void skipBytes(int total) {
        throw new RuntimeException("_LEGACY_ERROR_TEMP_3188");
    }
