...
public class BitSliceIndexRoaringBitmapTest {
    ...
    public void setup() throws IOException {
        ...
        BitSliceIndexRoaringBitmap.Builder builder = new BitSliceIndexRoaringBitmap.Builder(base, toPredicate(100));
        IntStream.range(0, 31).forEach(x -> builder.set(x, toPredicate(x)));
        IntStream.range(51, 100).forEach(x -> builder.set(x, toPredicate(x)));
        builder.set(100, toPredicate(30));
        this.bsi = builder.build();
    }

    @Test
    public void testSerde() throws IOException {
        BitSliceIndexRoaringBitmap.Builder builder = new BitSliceIndexRoaringBitmap.Builder(0, toPredicate(100));
        IntStream.range(0, 31).forEach(x -> builder.set(x, toPredicate(x)));
        IntStream.range(51, 100).forEach(x -> builder.set(x, toPredicate(x)));
        builder.set(100, toPredicate(30));
        byte[] serialize = builder.serialize();
        assertThat(BitSliceIndexRoaringBitmap.map(serialize)).isEqualTo(builder.build());
    }
